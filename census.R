##census api code inspired from http://rstudio-pubs-static.s3.amazonaws.com/19337_2e7f827190514c569ea136db788ce850.html
...
vars <- paste('B03002_', sprintf('%03i', seq(1, 20)), 'E', sep='')

getCensusTables<-function(stateFIPS,countyFIPS,table,lengthoftable){
  vars<-paste0(table,'_',sprintf('%03i', seq(1, lengthoftable)),'E')
  region<-paste0('for=tract:*&in=state:',stateFIPS,'+county:',countyFIPS)
  getCensusApi("http://api.census.gov/data/2014/acs5",vars=vars,region=region)
}

fipscodes<-read.csv("state_county_fips_codes_nfl.csv")
tables<-read.csv("tables_nfl.csv")

stateFIPS=fipscodes$State_FIPS
countyFIPS=fipscodes$County_FIPS
table=tables$Table
lengthoftable=tables$Length



table<-as.character(table)

##nfl_list<-mapply(getCensusTables,stateFIPS,countyFIPS,table,lengthoftable,SIMPLIFY = F)



list1<-list()
for(i in 1:5){
  t<-getCensusTabwarles(stateFIPS[i],countyFIPS[i],table[1],lengthoftable[1])
  list1[i]<-t
}


curl http://www2.census.gov/acs2013_5yr/summaryfile/2009-2013_ACSSF_By_State_By_Sequence_Table_Subset/Alabama/Tracts_Block_Groups_Only/20135al0001000.zip  -o mozart_piano_sonata.zip
